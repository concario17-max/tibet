...
const Player = ({ track, isPlaying, progress, onToggle, onNext, onPrev, onSeek, onClose }) => {
    ...
    return (
        <div className="fixed bottom-0 left-0 w-full z-[100] px-8 py-6 animate-in slide-in-from-bottom duration-700">
            <div className="max-w-7xl mx-auto glass-panel p-6 shadow-2xl flex flex-col md:flex-row items-center gap-8 border-gold-dim/40">

                {/* Track Info */}
                <div className="flex items-center space-x-6 w-full md:w-1/4">
                    <div className="w-12 h-12 bg-gold-primary/10 border border-gold-primary/20 flex items-center justify-center rounded-sm">
                        <Music className="text-gold-primary" size={20} strokeWidth={1.5} />
                    </div>
                    <div className="min-w-0">
                        <p className="text-[10px] tracking-widest text-gold-primary uppercase font-medium">Listening Now</p>
                        <h4 className="text-sm font-serif text-text-primary truncate uppercase tracking-wide">{track.title}</h4>
                    </div>
                </div>
                ...
                <div className="flex-1 w-full space-y-4">
                    <div className="flex items-center justify-center space-x-10">
                        ...
                        <button
                            onClick={onToggle}
                            className="w-12 h-12 rounded-full border border-gold-primary flex items-center justify-center text-gold-primary hover:bg-gold-primary hover:text-white-soft transition-all duration-500"
                        >
                    ...
                    <div className="relative group">
                        <div
                            className="w-full h-[2px] bg-charcoal/5 cursor-pointer relative"
                            onClick={(e) => {
                ...
                <div className="w-full md:w-1/4 flex items-center justify-end space-x-6 text-text-primary/40">
                    ...
                    <div className="w-24 h-px bg-charcoal/10 relative">
                        <div className="absolute left-0 top-0 h-full bg-gold-primary/40 w-2/3"></div>
                    </div>
